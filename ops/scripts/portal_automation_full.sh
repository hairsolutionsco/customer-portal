...
#   bash customer-portal/ops/scripts/op_env.sh bash customer-portal/ops/scripts/portal_automation_full.sh "chore(portal): nightly automation"
#   bash customer-portal/ops/scripts/portal_automation_full.sh --continue-on-error "chore(portal): msg"
#   SKIP_CONTACT_PROPS=1 bash customer-portal/ops/scripts/portal_automation_full.sh "docs: skip props"
...
if [[ "$SKIP_COMPLETE" == "1" ]]; then
  ...
  step "portal_task_complete (issues + HubSpot upload + git)" \
    bash "$SCRIPT_DIR/portal_task_complete.sh" "$COMMIT_MSG"
fi
