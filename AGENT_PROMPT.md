...
| **4. Next-step handoff in repo** | In **`docs/AGENT_PROMPT.md`** → *Portal orchestration* → **Next session - do this now** (search **`ORCHESTRATOR_HANDOFF`**, **`Next session`**, or **`do this now`**), update **only** that subsection (snapshot, **Lead agent - run in order**, **Subagents to launch**, **Blockers / do not launch until**) so the next agent knows what to do. Do **not** change other parts of **`docs/AGENT_PROMPT.md`** or this file solely for handoff text. |
