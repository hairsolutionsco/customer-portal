...
- **HubSpot CLI upload:** If `hs cms upload` errors with unknown arguments, use top-level **`hs upload <src> <dest> -m draft|publish`** from `customer-portal/cms` (account **50966981**). `portal_task_complete.sh` prefers `hs upload` when that command is available.
