...
**This document is the only orchestration plan that matters.** Treat root `HANDOFF_PROMPT.md`, `IMPLEMENTATION_PLAN_SUBAGENTS.md`, other roadmaps, Claude plan files, and ad-hoc instructions as **secondary** unless they align with this file. Cherry-pick from prior builds (`hair-solutions-portal/`, legacy Next.js, older theme trees) **only when necessary** to unblock work defined here — never by default.

## Context

**What:** Full orchestration blueprint for building out the Hair Solutions customer + admin portal theme under `cms/` of this repo, using parallel sub-agents in git worktrees, to ship on HubSpot Content Hub Professional (portal 50966981).
...
**Desired outcome:** One cohesive theme uploaded to Design Manager as `hair-solutions-portal`, matching the BASE + Inter visual language, covering 14 customer routes + 6 auth/system routes + 24 admin routes, with both a customer "My Profile" and an admin "My Profile" as separate self-edit pages.

### Locked-in decisions

| Decision                  | Answer                                                                                                                                                                                                                                                          |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Theme tree reconciliation | **Merge into `./cms/`** — but merge is actually pure pruning because `cms/` is already the newer superset                                                                                                                                                       |
| IA strategy               | **Hybrid**: keep Dashboard, Orders, Shop (Shopify web component embed), Billing (Stripe portal link), Invoices, Customization, Profile; **delete** Locations + probably Help/Support (fold into KB); **add** Tickets, Quotes, Documents, Events, Knowledge Base |
...
2. **The nested `./customer-portal/` directory contains four different things**:
   ...
   - (b) Stale doc copies (`AGENT_PROMPT.md`, `IMPLEMENTATION_PLAN_SUBAGENTS.md`, `HANDOFF_PROMPT.md`, `KNOWN_ISSUES.md`, `master-plan`) — authoritative versions live in `./docs/`
...
4. **Every `cms/` subdirectory has a tracked `.DS_Store`** from the initial commit. Adds diff noise to every PR.
...
| Path                                                        | Role                                                                                                                  |
| ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `cms/fields.json`                                           | Theme config fields. **High-contention file** — must be split into per-agent group namespaces (see G-ownership below) |
| `cms/theme.json`                                            | Theme metadata + preview template                                                                                     |
| `cms/css/main.css`, `portal-layout.css`, `components/*.css` | Design tokens + layout + component styles                                                                             |
| `cms/js/main.js`                                            | Sidebar toggle + mobile behavior                                                                                      |
| `cms/templates/layouts/portal.html`                         | Current customer shell (will be superseded by `portal-shell.html`)                                                    |
| `cms/templates/layouts/base.html`                           | Base HTML + serverless ping hook                                                                                      |
| `cms/templates/portal-*.html`                               | 12 customer page templates (some kept, some replaced)                                                                 |
| `cms/modules/**/*.module/`                                  | 20 existing modules across navigation, dashboard, orders, billing, catalog, profile, customization, settings          |
| `cms/data-queries/*.graphql`                                | 10 queries, contact-scoped; `orders_list`/`dashboard`/`order_detail` use the deals-as-orders alias                    |
...
- **Inter is self-hosted** under `cms/fonts/` — no CDN hand-wave. CSP on membership subdomain cannot be relied on for Google Fonts.
...
| **A0b** | `chore/portal-theme-prune`    | Delete: `cms/data-queries/locations.graphql`, `cms/templates/portal-locations.html`, `cms/modules/location-cards.module/` (verify exact path), `cms/templates/portal-help.html`, `cms/templates/portal-support.html`. Remove `locations_table_id` references from `cms/fields.json` + `portal-sidebar.module/fields.json`. Remove sidebar nav links to the deleted pages. Explicitly **do not** cherry-pick from the nested `src/` scaffold — PR description must quote "verified `cms/` is the newer superset; nested scaffold is stub-heavy and older."                                                                                           |
...
| ID     | Branch                         | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A1** | `feat/portal-theme-foundation` | Finalize `cms/fields.json` — owns the `portal`, `portal_layout`, `hubdb`, `typography` groups; publishes the **`fields.json` group ownership map** in the PR description. Self-host Inter 400/500/600/700 as `.woff2` under `cms/fonts/` with `@font-face` in `cms/css/main.css`. Finalize `:root` tokens (colors per §3.2 of the build prompt, radius, shadow, spacing scale). Refine `cms/css/portal-layout.css` layout primitives and `cms/css/components/*.css`. Refine `cms/js/main.js`.                                                                                                                                                                                                                                                                           |
| **A2** | `feat/portal-chrome`           | Create `cms/templates/layouts/portal-shell.html` — one layout accepting a `shell_mode` block variable (`customer` \| `admin`) that selects which sidebar module to render. Keep existing `portal.html` as a thin back-compat extend during A3–A6. Refine `cms/modules/navigation/portal-sidebar.module` (customer nav: Dashboard, Tickets, Quotes, Invoices, Documents, Events, Knowledge Base, Orders, Shop, Billing, Customization, My Profile). Create `cms/modules/navigation/portal-admin-sidebar.module` with the admin IA (dashboard, CRM, commerce, service, settings). Refine `cms/modules/navigation/portal-header.module` with user-menu link set variation. New partials: `cms/templates/partials/breadcrumbs.html`, `empty-state.html`, `pagination.html`. |

### Wave 2 — Auth + new queries (parallel)

| ID      | Branch                    | Scope                                                                                                                                                                                                                                                                                                                                                                           |
| ------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A3**  | `feat/portal-system`      | System templates refined against BASE + Inter: `membership-login.html`, `membership-register.html`, `membership-reset-password-request.html`, `membership-reset-password.html`, `404.html`, `500.html`. Refine `cms/css/templates/system.css`.                                                                                                                                  |
...
| ID      | Branch                          | Scope                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A4**  | `feat/portal-customer-core`     | Refine `cms/templates/portal-dashboard.html` + `dashboard-stats`, `production-alert`, `recent-orders`, `quick-actions` modules. Refine `cms/templates/portal-profile.html` + `hair-profile-display.module`, `hair-profile-form.module`. Embed HubSpot Form for profile edits (form GUID recorded in PR description). Deliver the **DND re-save list** in PR description (pages that need to be re-saved in Design Manager after upload because they use `{% dnd_area %}`).                                                                                                                                                                                                                                                                                      |
| **A5a** | `feat/portal-customer-service`  | Customer tickets + KB shell. Templates: `portal-tickets.html`, `portal-ticket-new.html`, `portal-ticket-detail.html`, `portal-knowledge-base.html`, `portal-knowledge-base-article.html`. Modules: `modules/service/ticket-list.module` (accepts `query_scope` field for A8 reuse), `ticket-detail.module`, `ticket-form.module` (HubSpot Form embed), `kb-search.module`, `kb-article.module`. **Requires a KB-mode decision from the orchestrator up front**: native HubSpot Service Hub KB (iframe/link shells) OR custom HubL KB. If native, A5a's KB scope shrinks to link shells.                                                                                                                                                                         |
...
2. **KB strategy** (A5a): native HubSpot Service Hub KB (iframe/link shells — recommended to avoid duplicating KB tooling) OR custom HubL KB pages.
...
| A0b   | `data/SCHEMA_REGISTRY.md`, `docs/AGENT_PROMPT.md` §1, `cms/fields.json`, `cms/data-queries/locations.graphql`, `cms/modules/portal-sidebar.module/{module.html,fields.json}`, `cms/templates/portal-{help,support,locations}.html`, `cms/modules/location-cards.module/*` |
| A9a   | `data/SCHEMA_REGISTRY.md` (§GraphQL association aliases), `docs/AGENT_PROMPT.md`, `docs/KNOWN_ISSUES.md`, every file in `cms/data-queries/`, build-prompt §6                                                                                                              |
| A1    | Build-prompt §3, `docs/base-theme-pro-style.pdf`, `cms/css/main.css`, `portal-layout.css`, `components/*.css`, `cms/{fields.json,theme.json}`, `cms/js/main.js`                                                                                                           |
| A2    | Build-prompt §3, §4.1, §4.3, all 9 customer screenshots + all 24 admin screenshots, `cms/templates/layouts/{portal,base}.html`, `cms/modules/navigation/portal-{sidebar,header}.module/`, A1 PR description                                                               |
| A3    | `docs/AGENT_PROMPT.md` §7, `cms/templates/system/*`, `cms/css/templates/system.css`, A1 + A2 PRs, `docs/base-theme-pro-style.pdf`                                                                                                                                         |
| A9b   | A9a results, `docs/AGENT_PROMPT.md`, all existing `cms/data-queries/*.graphql`, customer + admin screenshots for tickets/quotes/documents/events/KB                                                                                                                       |
| A4    | `docs/AGENT_PROMPT.md` §7, `cms/templates/portal-{dashboard,profile}.html`, existing dashboard + profile modules, `cms/data-queries/{dashboard,hair_profile}.graphql`, customer dashboard + profile screenshots                                                           |
...
| A6    | `data/SCHEMA_REGISTRY.md` (orders-as-deals section), `cms/templates/portal-{orders,order-detail,invoices,shop,billing,customization}.html`, existing commerce modules, Shopify CSP + Stripe URL decision memos                                                            |
