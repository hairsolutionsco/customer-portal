...
**Cursor / IDE:** Open **`hubspot/hubspot.code-workspace`** (repo root) or the **`hubspot/`** folder so tasks and paths resolve. For Design Manager–only windows, **`99-development/design-manager/customer-portal.code-workspace`** still works. Orchestration rules live in this file plus **`IMPLEMENTATION_PLAN_SUBAGENTS.md`** and **`AGENT_PROMPT.md`**; secrets workflow is **`hubspot/AGENTS.md`** (`bash scripts/op_run.sh …`). You do **not** need to paste the full handoff every time if §1 below is current.
...
- **Repo:** `hubspot/99-development/design-manager` · CMS theme: `customer-portal/cms/` · Next.js app: `customer-portal/app/`
- **Issues:** #3–#57 · exports: `customer-portal/ops/exports/github-issues.json` (run `npm run portal:issues` from hubspot or design-manager root to refresh)
...
- **Baseline:** GraphQL in repo targets **deals-as-orders** (`deal_collection__contact_to_deal` — verify in explorer), **HubDB** table names `products`, `affiliated_locations`, `subscription_plans`, and contact JSON fields `portal_hair_profile_json`, `portal_saved_templates_json`, `portal_invoices_json`. **API (required):** **`bash scripts/op_run.sh npm run portal:hubspot-props`** and **`bash scripts/op_run.sh npm run portal:hubdb-sync`** from **hubspot** root (or `bash customer-portal/ops/scripts/op_env.sh npm run …` from **design-manager** root). **Next gate:** upload theme; fix association labels + HubDB column names if validation errors.
...
4. After the batch: **`bash customer-portal/ops/scripts/portal_task_complete.sh "type(scope): what completed"`** from **design-manager** root (or `bash scripts/op_run.sh bash 99-development/design-manager/customer-portal/ops/scripts/portal_task_complete.sh "…"` from **hubspot** root). Use `SKIP_HUBSPOT=1` only if you must skip upload. Upload uses **`hs`** with default account from **`~/.hscli/config.yml`**; optional local `customer-portal/cms/hubspot.config.yml` is gitignored. See **§3 Task completion ritual**.
...
- **A5/A6:** `customer-portal/data/SCHEMA_REGISTRY.md` populated enough to match explorer names (see plan handoff).
...
4. Work only in your scope. Update `customer-portal/data/SCHEMA_REGISTRY.md` if CRM/HubDB IDs or GraphQL association names change.
...
Deliver **Portal 2.0** on **HubSpot CMS** under `customer-portal/cms/`. The Next.js app lives in `customer-portal/app/` (external hosting; not uploaded to Design Manager).
...
| Path | Purpose |
|------|---------|
| `customer-portal/cms/` | CMS theme source (`hs cms upload`; flat layout — `portal_task_complete.sh` uses `.` or `src` as needed) |
| `customer-portal/data/schemas/` | Schema JSON (contact props / reference; HubDB seeds under `hubdb/`) |
| `customer-portal/data/hubdb/` | HubDB seed JSON (sync via `portal:hubdb-sync`) |
| `customer-portal/data/SCHEMA_REGISTRY.md` | Live IDs + GraphQL names (**required** before A5/A6 finalize) |
| `customer-portal/app/` | Next.js customer portal app |
| `customer-portal/ops/scripts/` | Ship ritual, HubDB/props Python, GitHub exports |

**GitHub:** `hairsolutionsco/customer-portal` (issues + PRs; this monorepo path is under `hubspot/`)
...
```bash
bash customer-portal/ops/scripts/portal_task_complete.sh "type(scope): what completed"
```

From **`hubspot/`** (engineering monorepo root):

```bash
bash scripts/op_run.sh bash 99-development/design-manager/customer-portal/ops/scripts/portal_task_complete.sh "type(scope): what completed"
```

Refreshes issue exports under `customer-portal/ops/exports/`, `hs` theme upload (if CLI auth OK), git commit + push at **`hubspot/`** git root. Flags: `SKIP_HUBSPOT=1`, etc. — see **`IMPLEMENTATION_PLAN_SUBAGENTS.md` §6a**.

**Secrets:** Never print PAKs; never commit `customer-portal/cms/hubspot.config.yml` or `.env` (`.env` is gitignored at `hubspot/`). Optional local private-app tokens live in `.env` only — use **`HUBSPOT_PRIVATE_APP__CRM_SCHEMA__ACCESS_TOKEN`**, **`HUBSPOT_PRIVATE_APP__HUBDB__ACCESS_TOKEN`**, or shared **`HUBSPOT_PRIVATE_APP__OPS__ACCESS_TOKEN`**.

### HubSpot CLI and portal choice (#3–#5)

**Default:** credentials live in **`~/.hscli/config.yml`**. If **`hs accounts list`** already shows your portal and uploads work, **do nothing** — auth lasts until you revoke the key or use a new machine. Only run **`hs account auth`** when you need to (first setup, rotated personal access key, new account, or CLI says you are not logged in). Theme upload from `customer-portal/cms/`: use **`portal_task_complete.sh`** (handles `hs cms upload` vs legacy `hs upload`, and **`.` vs `src`**). Manual examples: **`hs cms upload . hair-solutions-portal`** (flat theme) or **`hs cms upload src hair-solutions-portal`** if your theme uses a `src/` folder.
...
4. **`customer-portal/ops/exports/github-issues.json`** — AC per issue
...
| Prerequisite | Why |
|--------------|-----|
| **`hs account auth`** done; **`hs accounts list`** shows default; **`hs upload`** / **`hs cms upload`** works once manually from `customer-portal/cms/` | Unblocks ritual uploads |
...
**No-AI automation:** You can cron `npm run portal:issues` to keep `customer-portal/ops/exports/github-issues.json` fresh; that alone does **not** implement features or close HubSpot-side work.
